    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-6">
      <div class="relative">
        ...
        <div v-if="images.length > 1" class="mt-3 grid grid-cols-5 gap-3">
          <button v-for="(img, index) in images" :key="index" @click="currentIndex = index" class="aspect-w-1 aspect-h-1 rounded-md overflow-hidden ring-2 transition-all duration-200" :class="currentIndex === index ? 'ring-brand-primary ring-offset-2 dark:ring-offset-gray-800' : 'ring-transparent hover:ring-brand-primary/50'">
      ...
      <div>
        <div class="mb-4">
          <span class="inline-block bg-brand-light text-brand-dark text-xs font-semibold px-2.5 py-1 rounded-full mb-2">{{ product.category_name }}</span>
          <h1 id="product-modal-title" class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white leading-tight">{{ product.title }}</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">por <span class="font-medium">{{ product.user_username }}</span></p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm mb-6">
          <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
            ...
            <dd class="font-semibold text-gray-800 dark:text-gray-100 mt-0.5">{{ product.condition }}</dd>
          </div>
          <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
            <dt class="font-medium text-gray-500 dark:text-gray-400">Disponibilidad</dt>
            <dd class="font-semibold text-gray-800 dark:text-gray-100 mt-0.5">{{ product.status === 'available' ? 'Disponible' : 'No disponible' }}</dd>
          </div>
        </div>
        <div v-if="product.exchange_interests && product.exchange_interests.length > 0" class="mb-6">
          ...
          <div class="flex flex-wrap gap-2">
            <span v-for="interest in product.exchange_interests" :key="interest" class="bg-white dark:bg-gray-700/60 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-200 text-xs font-semibold px-3 py-1.5 rounded-full">
              {{ interest }}
            </span>
          </div>
        </div>
        <div class="pt-6 border-t border-gray-200 dark:border-gray-700">
          ...
          <div v-else-if="userStore.user.id === product.user_id">
            <p class="text-sm text-center text-gray-600 dark:text-gray-300 bg-yellow-50 dark:bg-yellow-800/20 p-4 rounded-lg">Este es tu propio producto.</p>
          </div>
          <div v-else>
            <label for="my-product-select" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Selecciona tu producto a ofrecer:</label>
            ...
            <div v-else-if="userProducts.length === 0" class="text-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
              <p class="text-sm text-gray-600 dark:text-gray-300">No tienes productos disponibles para intercambiar.</p>
            ...
            <div v-else class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <select id="my-product-select" v-model="selectedUserProductId" class="block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-brand-primary focus:ring-brand-primary sm:text-sm h-11">
              ...
              <button
                ...
                class="w-full sm:w-auto px-5 py-2.5 text-sm font-semibold text-white bg-brand-primary rounded-md shadow-sm hover:bg-brand-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-primary disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center transition-colors duration-200"
              >
...
// --- CAMBIO 1: Obtener la URL base de la API desde las variables de entorno ---
...
// --- CAMBIO 2: Lógica de normalización de URL mejorada ---
function normalizeImageUrl(url) {
  if (!url) return placeholderImage;
  // Si la URL ya es absoluta (empieza con http, https o es data:), no la modificamos.
  if (/^(https?:|data:)/i.test(url)) {
    return url;
  }
  // Si es una ruta relativa (ej: /media/...), le añadimos la URL base de la API.
